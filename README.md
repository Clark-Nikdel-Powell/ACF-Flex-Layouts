...
* A `acf-flex-layouts` directory that contains the source code - a fully executable WordPress plugin.
...
* `acf-flex-layouts/includes` is where functionality shared between the admin area and the public-facing parts of the site reside
* `acf-flex-layouts/admin` is for all admin-specific functionality
* `acf-flex-layouts/public` is for all public-facing functionality

Note that previous versions of the Boilerplate did not include `ACF_Flex_Layouts_Loader` but this class is used to register all filters and actions with WordPress.
